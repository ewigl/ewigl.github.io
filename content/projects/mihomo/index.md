...
3. 双击 `mihomo.start.vbs` 在后台运行，

   双击 `mihomo.start.bat` 在前台运行（会始终显示一个"小黑框"）。
...
后台运行时：双击运行 `mihomo.stop.bat`，或打开任务管理器，结束 `mihomo-windows-amd64.exe` 进程。
...
2.  在模块管理界面, 可以使用“执行”按钮来运行、停止 mihomo。
3.  Log 文件位于 `/data/adb/box/run` 目录中。

## 提醒

这是实现 Tun 模式的简单配置，使用默认 DNS，使用官方规则集，更多定制功能请参考[官方文档](https://wiki.metacubex.one/)。
